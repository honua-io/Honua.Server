...
using Honua.Server.Services.Models.Import;

namespace Honua.Server.Services.Services.Import;
    ...
    public JsonElement GenerateMapLibreStyle(StyleDefinition style, string layerId, string sourceId)
    {
        ...
        if (style.PointStyle != null)
        {
            layers.Add(new
            {
                ...
                type = "circle",
                source = sourceId,
                filter = new object[] { "==", new object[] { "geometry-type" }, "Point" },
                paint = new
                {
        ...
        if (style.LineStyle != null)
        {
            layers.Add(new
            {
                ...
                type = "line",
                source = sourceId,
                filter = new object[] { "==", new object[] { "geometry-type" }, "LineString" },
                layout = new
                {
        ...
        if (style.PolygonStyle != null)
        {
            layers.Add(new
            {
                ...
                type = "fill",
                source = sourceId,
                filter = new object[] { "==", new object[] { "geometry-type" }, "Polygon" },
                paint = new
                {
