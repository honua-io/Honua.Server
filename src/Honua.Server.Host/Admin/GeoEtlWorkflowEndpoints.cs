...
public static class GeoEtlWorkflowEndpoints
{
    public static IEndpointRouteBuilder MapGeoEtlWorkflowEndpoints(this IEndpointRouteBuilder endpoints)
    {
            ...
            [FromQuery] int offset = 0) =>
        {
            ...
            var workflows = await store.ListWorkflowsByTenantAsync(tenantId.Value, limit, offset);
            ...
            [FromQuery] Guid? tenantId) =>
        {
            ...
            var workflow = await store.GetWorkflowAsync(workflowId, tenantId.Value);
            if (workflow == null)
            {
            ...
            [FromBody] CreateWorkflowRequest request) =>
        {
            ...
            var created = await store.CreateWorkflowAsync(workflow, request.CreatedBy);
            return Results.Created($"/admin/api/geoetl/workflows/{created.WorkflowId}", created);
        })
            ...
            [FromBody] UpdateWorkflowRequest request) =>
        {
            ...
            var existing = await store.GetWorkflowAsync(workflowId, request.TenantId);
            if (existing == null)
            {
                return Results.NotFound(new { error = "Workflow not found" });
            }
            ...
            var updated = await store.UpdateWorkflowAsync(existing, request.UpdatedBy);
            ...
            [FromQuery] Guid? deletedBy) =>
        {
            ...
            await store.DeleteWorkflowAsync(workflowId, deletedBy.Value);
            ...
            [FromQuery] Guid? tenantId) =>
        {
            ...
            var validation = await engine.ValidateAsync(workflow);
            ...
            [FromQuery] Guid? tenantId) =>
        {
            ...
            var estimate = await engine.EstimateAsync(workflow);
