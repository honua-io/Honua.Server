...
/// <summary>
/// Handles authorization for layer resources based on configured policies.
/// </summary>
public sealed class LayerAuthorizationHandler : IResourceAuthorizationHandler
{
    private readonly IResourceAuthorizationCache _cache;
    private readonly ResourceAuthorizationMetrics _metrics;
    private readonly ILogger<LayerAuthorizationHandler> _logger;
    private readonly IOptionsMonitor<ResourceAuthorizationOptions> _options;

    public string ResourceType => "layer";

    public LayerAuthorizationHandler(
        IResourceAuthorizationCache cache,
        ResourceAuthorizationMetrics metrics,
        ILogger<LayerAuthorizationHandler> logger,
        ...
        return await PerformanceMeasurement.MeasureAsync(
            ...
            async () =>
            {
                try
                {
                    ...
                    if (!options.Enabled)
                    {
                        _logger.LogTrace("Resource authorization is disabled, allowing access to layer {LayerId}", resourceId);
                ...
                {
                    _logger.LogError(ex, "Error during authorization check for layer {LayerId}, operation {Operation}", resourceId, operation);
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (applicablePolicies.Count > 0)
        {
            ...
            _logger.LogDebug(
                "Layer authorization granted for {UserId} on layer {LayerId} (operation: {Operation}) via policy {PolicyId}",
                userId,
                resourceId,
                operation,
                matchedPolicy.Id);
        ...
        {
            _logger.LogDebug(
                "Layer authorization granted for administrator {UserId} on layer {LayerId} (operation: {Operation})",
                userId,
                resourceId,
                operation);
            return Task.FromResult(ResourceAuthorizationResult.Success());
        ...
        {
            _logger.LogDebug(
                "Layer authorization granted by default policy for {UserId} on layer {LayerId} (operation: {Operation})",
                userId,
                resourceId,
                operation);
            return Task.FromResult(ResourceAuthorizationResult.Success());
        ...
        _logger.LogWarning(
            "Layer authorization denied for {UserId} on layer {LayerId} (operation: {Operation})",
            userId,
            resourceId,
            operation);
