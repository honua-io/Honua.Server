...
-- Description: Adds specialized indexes for temporal range queries using expressions
...
-- SQLite supports expression-based indexes natively (added in SQLite 3.9.0)
-- This allows us to create indexes on COALESCE expressions directly
...
-- SQLite automatically analyzes tables, but we can explicitly trigger it
ANALYZE stac_items;
...
-- 6. SQLite supports both expression-based indexes and partial indexes (WHERE clauses)
-- 7. Query planner will automatically choose the most efficient index based on the query pattern
-- 8. Use EXPLAIN QUERY PLAN to verify index usage in your queries
